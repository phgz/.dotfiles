...
api.nvim_create_autocmd("CmdlineLeave", {
	callback = function()
		...
		if (event_info.cmdtype == "/" or event_info.cmdtype == "?") and not event_info.abort then
			-- vim.notify("No match found for: %#RedStatusLine#" .. fn.getreg("/"))
			vim.schedule(function()
				local result = require("utils").searchcount()
				vim.notify(result and result or "No match found for: %#RedStatusLine#" .. fn.getreg("/"))
			end)
		end
	end,
})
