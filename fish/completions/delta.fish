...
complete -c delta -l detect-dark-light -d 'Detect whether or not the terminal is dark or light by querying for its colors' -r -f -a "auto\t'Only query the terminal for its colors if the output is not redirected'
...
complete -c delta -l generate-completion -d 'Print completion file for the given shell' -r -f -a "bash\t''
elvish\t''
fish\t''
powershell\t''
zsh\t''"
...
complete -c delta -l grep-output-type -d 'Grep output format. Possible values: "ripgrep" - file name printed once, followed by matching lines within that file, each preceded by a line number. "classic" - file name:line number, followed by matching line. Default is "ripgrep" if `rg --json` format is detected, otherwise "classic"' -r -f -a "ripgrep\t''
classic\t''"
...
complete -c delta -l inspect-raw-lines -d 'Kill-switch for --color-moved support' -r -f -a "true\t''
false\t''"
complete -c delta -l line-buffer-size -d 'Size of internal line buffer' -r
complete -c delta -l line-fill-method -d 'Line-fill method in side-by-side mode' -r -f -a "ansi\t''
spaces\t''"
...
complete -c delta -l paging -d 'Whether to use a pager when displaying output' -r -f -a "auto\t''
always\t''
never\t''"
...
complete -c delta -l true-color -d 'Whether to emit 24-bit ("true color") RGB color codes' -r -f -a "auto\t''
always\t''
never\t''"
...
complete -c delta -l 24-bit-color -d 'Deprecated: use --true-color' -r -f -a "auto\t''
always\t''
never\t''"
